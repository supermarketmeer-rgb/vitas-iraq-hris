...
export const ExpiryManagement: React.FC<ExpiryManagementProps> = ({ theme = 'light' }) => {
  ...
  return (
    <div className={`text-center py-16 rounded-3xl border transition-all ${
      ...
      <h2 className="text-xl font-bold mb-2">
        {t('إدارة الانتهاء والتنبيهات التلقائية', 'Expiry & Alert Management')}
      </h2>
      <p className={`text-sm max-w-md mx-auto ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
        {t('متابعة صلاحية العقود، الهويات، والجوازات وتوليد التنبيهات الدورية قبل الانتهاء.', 'Track contract, passport, and ID validity with automated proactive notifications.')}
      </p>
    </div>
  );
};
